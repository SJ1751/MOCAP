...
void mpu9250::MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
 ...
 void mpu9250::getvalues(float Thevalues[3]){
  tcaselect(mux1port);
 ...
 bool mpu9250::ini(uint8_t _address , int32_t _mux1port){
  Serial.println("Inside ini");
  address=_address;
  mux1port=_mux1port;
  #define MPU9250_ADDRESS  address
   tcaselect(mux1port);
   bool mpuini=false;
   bool magini=false;
 ...
 void mpu9250::tcaselect(uint8_t i) {
  if (i > 7) return;
